...
내가 졸업할 때만 해도 인스타그램, 페이스북 트위터는 우리나라에서 많이 사용 안하는 편이였다.요즘은 다르다. `뉴스토픽에 어느정치인이, 연예인이 SNS에 글을 썻다`로 뉴스 토픽이 되는것이 요즘이다.
...
나는 모든 사람이 작가라고 생각한다. 사람은 자신이 주체가 되어서 자신의 삶을 살고 있기 때문이다. 사람이라함은 본능적으로 '난 누군가에게 관심(사랑)을 받고 싶어 하기 때문이다. 자신의 이야기로 업을 삼고 있는 이들이 `작가` 라고 생각이 든다. 그중에서 그림그리는 사람만이 작가[^writer]는 아니다. (글쓰는 사람을 소설 작가가 있듯이 말이다.)
