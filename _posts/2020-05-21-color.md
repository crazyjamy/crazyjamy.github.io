...
+ 단색(Monochromatic): 중요한 정보를 주색으로 사용하고, 회색 영역도 주색과 같은 계열의 톤을 사용합니다.
+ 유사색(Analogous): 비슷한 개념이지만, 구분이 필요한 상황일 떄 유사색을 사용합니다.
+ 보색(Complementary): 다른 요소보다 강력하게 강조할 필요가 있을 때 사용합니다.

## 색 비율
인테리어에서 사용되는 60-30-10 법칙을 활용할 수 있습니다. 60% 30%를 활용해서 배경색을 배분하고 강조하고자한느 요소나 글 위주의 서비스일 경우 10%를 배분합니다. 전체의 량을 생각하며 배경색을 배분한뒤 포인트 컬러를 10% 내에서 조금씩 추가하는 방식으로 색 비율을 조정합니다. 
